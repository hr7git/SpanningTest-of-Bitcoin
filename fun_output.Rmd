...
</br>Table 4.2 Asset allocations of HMC Portfolio Set 
```{r,echo=FALSE,message=FALSE }
...
<Table 4.2> shows Harvard University's fund portfolio comprising stocks, bonds, and real estate. To represent these assets, the ETF portfolio in Table 4.3 is used as a benchmark asset set.
...
# benchmark_set <- read_csv("benchmark_set.csv")
benchmark_set <- read_csv("https://www.dropbox.com/s/z42btvylghq6lr8/benchmark_set.csv?dl=0")
# https://www.dropbox.com/s/z42btvylghq6lr8/benchmark_set.csv?dl=0

kable(head(benchmark_set,11),
...
<!-- </br>Table 4.3 The Ivy 10’s Asset Allocation   -->
<!-- ```{r,echo=FALSE,message=FALSE } -->
<!-- df_table <- read_excel("Ivy Asset Allocation.xlsx",sheet = "ivy10") -->
<!-- kable(head(df_table,12), -->
...
```{r}
 library(quantmod)
 chart_Series(rets$BTC)
 chart_Series(rets$SPY)
 chart_Series(rets$GLD)
 # chartSeries(`BTC`)
 # chartSeries(`ETH`)
```
